...
contract CASH is ERC20, Ownable {
    ...
    CASHDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = BNBCASHFee.add(liquidityFee).add(marketingFee).add(dPrizeFee).add(mPrizeFee).add(yPrizeFee);

    address public marketingWallet = 0xAf8c9916f264FD8Fd45EAb69A93A6494156eF1A6;
    address public devWallet;
    address public dPrizeWallet;
    address public mPrizeWallet;
    address public yPrizeWallet;
    ...
    constructor() ERC20("CASHBACK", "CASH") {
    	...
    	dividendTracker = new CASHDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CASH: The dividend tracker already has that address");

        CASHDividendTracker newDividendTracker = CASHDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CASH: The new dividend tracker must be owned by the CASH token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CASH: The router already has that address");
    ...
    function setBNBCASHFee(uint256 value) external onlyOwner{
        BNBCASHFee = value;
        totalFees = BNBCASHFee.add(liquidityFee).add(marketingFee).add(dPrizeFee).add(mPrizeFee).add(yPrizeFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CASH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CASH: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CASH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CASH: Cannot update gasForProcessing to same value");
    ...
    function airdropTokens(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
        require(accounts.length == amounts.length, "Arrayts must have same size");
        for(uint256 i = 0; i < accounts.length; i++){
            super._transfer(msg.sender, accounts[i], amounts[i]*10**9);
            try dividendTracker.setBalance(payable(accounts[i]), balanceOf(accounts[i])) {} catch {}
        }
    }
    ...
    function _transfer(
        ...
        if(_isPresalers[from]) {
            if(block.timestamp.sub(_firstSell[from]) < 1 days){
                ...
                require(amount <= maxDailySellForInvestors, "You are exceeding maxDailySellForInvestors");
        ...
        if(!_isExcludedFromFees[from] && !automatedMarketMakerPairs[from] && !swapping){
            if(block.timestamp.sub(_firstSell[from]) < 1 days){
                ...
                _firstSell[from] = block.timestamp;
                require(amount <= maxDailySell, "You are exceeding maxDailySell");
                _dailySells[from] = amount;
            }
        }
        ...
        if( canSwap &&
            ...
            if(BNBCASHFee > 0){
                uint256 sellTokens = contractTokenBalance.mul(BNBCASHFee).div(totalFees);
...
contract CASHDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor()  DividendPayingToken("CASH_Dividen_Tracker", "CASH_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 200,000 tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CASH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CASH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CASH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CASH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CASH_Dividend_Tracker: Cannot update claimWait to same value");
